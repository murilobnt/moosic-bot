...
class MusicPlayer(commands.Cog):
    ...
    async def verify_user_voice(self, ctx):
        if not ctx.author.voice:
            raise MoosicError(await self.translator.translate("er_con", ctx.guild.id))

    async def verify_connection(self, queue):
        if not queue.get('connection'):
            raise MoosicError(await self.translator.translate("er_conb", ctx.guild.id))

    async def verify_info_fields(self, info):
        if not info.get('title') or not info.get('url'):
            raise MoosicError(await self.translator.translate("er_down", ctx.guild.id))
    ...
    async def play_song_index(self, ctx, queue, song_index):
        ...
        if url_int < 1 - modifier or url_int > len(queue['meta_list']) - modifier:
            raise MoosicError(await self.translator.translate("er_index", ctx.guild.id))
    ...
    async def handle_on_playlist_url(self, guild_id, queue, info, author_mention):
        ...
        if not parsed_query or not parsed_query.get('v'):
            raise MoosicError(await self.translator.translate("er_ytdl", ctx.guild.id))
        info = self.ytdl.extract_info("https://youtube.com/watch?v=bruh", download=False)
        await self.verify_info_fields(info)
    ...
    async def handle_input(self, guild_id, queue, url, author_mention):
        if validators.url(url):
            ...
            try:
                ...
                raise MoosicError(await self.translator.translate("er_url", guild_id))
    ...
    async def connect_and_play(self, ctx, queue):
        try:
            ...
            raise MoosicError(await self.translator.translate("er_conc", ctx.guild.id))
    ...
    async def play(self, ctx, *, url : str):
        ...
        await self.verify_user_voice(ctx)
        ...
        if not channel_permissions.connect or not channel_permissions.speak:
            raise MoosicError(await self.translator.translate("er_perm", ctx.guild.id))
        ...
        try:
            if song_index:
                await self.verify_connection(queue)
                await self.verify_no_songs(queue)
    ...
    async def enqueue_song(self, guild_id, queue, info, mention):
            ...
            song_add = await self.translator.translate("song_add", guild_id)
            description = song_add.format(title=meta.get('title'), url=meta.get('url'), mention=mention)
            embed = discord.Embed(
    ...
    async def enqueue_playlist(self, guild_id, queue, playlist_items, mention):
        ...
        pl_add = await self.translator.translate("pl_add", guild_id)
        description = pl_add.format(pl_len=len(playlist_items))
        embed = discord.Embed(
                description=description,
                color=0xcc0000)
        await queue['text_channel'].send(embed=embed)

    async def basic_verifications(self, ctx):
        await self.basic_verifications_without_songs(ctx)
        await self.verify_no_songs(self.servers_queues.get(ctx.guild.id))

    async def basic_verifications_without_songs(self, ctx):
        await self.verify_registry(ctx)
        await self.verify_same_voice(ctx)

    async def verify_is_playing(self, queue):
        if queue['song_index'] >= len(queue['meta_list']):
            raise MoosicError(await self.translator.translate("er_nosong", ctx.guild.id))
    ...
    async def skip(self, ctx, *, how_many : int = None):
        ...
        await self.basic_verifications(ctx)
        queue = self.servers_queues.get(ctx.guild.id)
        await self.verify_is_playing(queue)
        if how_many:
            try:
                how_many = int(how_many)
                if how_many <= 0:
                    raise MoosicError(await self.translator.translate("er_skipindex", ctx.guild.id))
            except ValueError:
                raise MoosicError(await self.translator.translate("er_skiparg", ctx.guild.id))
        else:
            how_many = 1
        ...
        skip_succ = await self.translator.translate("skip_succ", ctx.guild.id)
        description = skip_succ.format(how_many=how_many)
        embed = discord.Embed(
                description=description,
                color=0xcc0000)
    ...
    async def pause(self, ctx):
        ...
        if queue.get('paused_time'):
            raise MoosicError(await self.translator.translate("er_paused", ctx.guild.id))
    ...
    async def resume(self, ctx):
        ...
        if not queue.get('paused_time'):
            raise MoosicError(await self.translator.translate("er_nopause", ctx.guild.id))
    ...
    async def seek(self, ctx, timestamp : str):
        ...
        if not re.match('^(\d{1,2}:)?(\d{1,2}:)?(\d{1,2})$', timestamp) and not re.match('^\d+$', timestamp):
            raise MoosicError(await self.translator.translate("er_seekarg", ctx.guild.id))
    ...
    async def np(self, ctx):
        ...
        if not queue['elapsed_time']:
            raise MoosicError(await self.translator.translate("er_npdat", ctx.guild.id))
        ...
        if not live:
            ...
            np_succnorm = await self.translator.translate("np_succnorm", ctx.guild.id)
            description = np_succnorm.format(title=song.get('title'), weburl=song.get('web_url'), mention=ctx.author.mention, progress_bar=progress_bar, formatted_elapsed=formatted_elapsed, formatted_duration=formatted_duration)
            ...
            np_succlive = await self.translator.translate("np_succlive", ctx.guild.id)
            description = np_succlive.format(title=song.get('title'), weburl=song.get('web_url'), mention=ctx.author.mention, formatted_elapsed=formatted_elapsed)
            embed = discord.Embed(
                    description=description,
                    color=0xedd400)
            await ctx.send(embed=embed)
    ...
    async def queue(self, ctx):
        ...
        if il_index == "q_songrep":
            if queue['song_index'] < len(queue['meta_list']):
                in_loop = await self.translator.translate("q_next", ctx.guild.id)
            else:
                bruh = await self.translator.translate(il_index, ctx.guild.id)
                in_loop = bruh.format(song=queue['meta_list'][queue['song_index']]['title'])
        elif il_index == ":P":
            in_loop = ":P"
        else:
            in_loop = await self.translator.translate(il_index, ctx.guild.id)

        songs = await self.build_text(ctx.guild.id, meta_list, elapsed, song_index, page)
        current_page = await self.build_page(ctx.guild.id, songs, in_loop, page, last_page)

        msg = await ctx.send(await self.build_page(ctx.guild.id, songs, in_loop, page, last_page))
        if page == last_page:
            return
        ...
        while True:
            try:
                ...
                if str(reaction.emoji) == "▶️":
                    ...
                    await msg.edit(content=await self.build_page(ctx.guild.id, songs, in_loop, page, last_page))
    ...
    async def build_text(self, guild_id, meta_list, elapsed, song_index, page):
        songs = ""
        it = 1 + (page * 10)
        index = it
        ...
        for entry in meta_list[index - 1 : (index - 1) + 10]:
            ...
            if it == song_index + 1 and page == 0:
                if not live:
                    songs = songs + await self.translator.translate("q_np", guild_id)
                else:
                    songs = songs + await self.translator.translate("q_nplive", guild_id)
    ...
    async def build_page(self, guild_id, songs, in_loop, page, last_page):
        q_page = await self.translator.translate("q_page", guild_id)
        return q_page.format(page_plus=page + 1, last_page_plus = last_page + 1, songs=songs, in_loop = in_loop)
    ...
    async def disconnect(self, ctx):
        ...
        dc_message = await ctx.send(await self.translator.translate("dc_msg", ctx.guild.id))
    ...
    async def verify_same_voice(self, ctx):
        if ctx.author.voice and ctx.voice_client and ctx.author.voice.channel != ctx.voice_client.channel:
            raise MoosicError(await self.translator.translate("er_vsv", ctx.guild.id))

    async def verify_registry(self, ctx):
        if not self.servers_queues.get(ctx.guild.id):
            raise MoosicError(await self.translator.translate("er_vr", ctx.guild.id))

    async def verify_no_songs(self, queue):
        if not queue.get('meta_list'):
            raise MoosicError(await self.translator.translate("er_vns", ctx.guild.id))
    ...
    async def remove(self, ctx, index : int):
        ...
        if m_index >= len(songs_list):
            raise MoosicError(await self.translator.translate("er_rmindex", ctx.guild.id))
    ...
    async def loop(self, ctx):
        ...
        if queue['loop'] == LoopState.NOT_ON_LOOP:
            ...
            mode = await self.translator.translate("loop_lq", ctx.guild.id)
        elif queue['loop'] == LoopState.LOOP_QUEUE:
            queue['loop'] = LoopState.LOOP_TRACK
            mode = await self.translator.translate("loop_ls", ctx.guild.id)
        elif queue['loop'] == LoopState.LOOP_TRACK:
            queue['loop'] = LoopState.NOT_ON_LOOP
            mode = await self.translator.translate("loop_noloop", ctx.guild.id)

        loop_succ = await self.translator.translate("loop_succ", ctx.guild.id)
        description = loop_succ.format(mode=mode)
        embed = discord.Embed(
                description=description,
                color=0xcc0000)
        await ctx.send(embed=embed)
    ...
    async def play_songs(self, guild_id):
        ...
        if not queue['same_song']:
            try:
                ...
                er_playdl = await self.translator.translate("er_playdl", guild_id)
                skipping = er_playdl.format(title=song['title'])
                await text_channel.send(skipping)
            ...
            name = await self.translator.translate("play_np", guild_id)
            embed=discord.Embed(
    ...
    async def inactive(self, guild_id, queue):
        notice = await self.translator.translate("inactive_notice", guild_id)
        await self.halt(guild_id, queue, 180, notice)
        self.cancel_alone_task(queue)

    async def alone(self, guild_id, queue):
        notice = await self.translator.translate("alone_notice", guild_id)
        await self.halt(guild_id, queue, 60, alone_notice)
        self.cancel_halt_task(queue)
