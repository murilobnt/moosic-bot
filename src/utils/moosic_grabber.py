...
class PytubeMoosicVideo:
    ...
    def get_duration(self):
        return int(self.yt.length)

    def is_live(self):
        return self.yt.vid_info["videoDetails"]["isLiveContent"]

    def get_thumbnail(self):
        return self.yt.thumbnail_url

    def get_audio_url(self):
        return self.yt.streams.get_audio_only().url

class MoosicGrabber:
    @staticmethod
    def request_yt(video_id):
        return PytubeMoosicVideo(YouTube(f"https://www.youtube.com/watch?v={video_id}"))
